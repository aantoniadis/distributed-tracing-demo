...
public class TraceConfig {
  ...
  @Bean
  Executor executor(CurrentTraceContext traceContext) {
    ExecutorService executor = Executors.newFixedThreadPool(5);
    return traceContext.executor(executor);
  }
  ...
  Tracing tracing(CurrentTraceContext traceContext, Reporter reporter) {
    return Tracing.newBuilder()
        .localServiceName("service-demo-1")
        .reporter(reporter)
        .currentTraceContext(traceContext)
        .build();
  }
  ...
  FitFilter fitFilter(ClientHttpRequestInterceptor clientHttpRequestInterceptor,
      List<RestTemplate> templates) {
    return new FitFilter(clientHttpRequestInterceptor, templates);
  }
  ...
  static class FitFilter {
    ...
    FitFilter(ClientHttpRequestInterceptor interceptor, List<RestTemplate> restTemplates) {
      restTemplates.forEach(r -> r.getInterceptors().add(0, interceptor));
    }
  }
